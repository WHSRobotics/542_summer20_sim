...
/*
    public static StrafePath generateStrafePath(ArrayList<Position> targetPositions, org.whitneyrobotics.ftc.teamcode.lib.purepursuit.FollowerConstants lookaheadDistance, StrafePathGenerationConstants constants) {
        ArrayList<Position> positionList = generatePosPath(targetPositions, constants.getSpacing(), constants.getWeightSmooth());
        ArrayList<Coordinate> coordinateList = generateCoordPath(targetPositions, constants.getSpacing(), constants.getWeightSmooth());
        double[] targetTangentialVelocities = calculateTargetTangentialVelocities(constants.getTurnSpeed(), constants.getPathMaxVelocity(), org.whitneyrobotics.ftc.teamcode.lib.purepursuit.PurePursuitRobotConstants.MAX_ACCELERATION, positionList);
...
/*    private static ArrayList<Coordinate> generateCoordPath(ArrayList<Position> targetPositions, double spacing, double weightSmooth) {
    ...
    private static ArrayList<Position> smoothPath(ArrayList<Position> orig, double weightSmooth) {
        ...
        for(Position pos : orig){
            smoothed.add(new Position(pos.getX(), pos.getY()));
        }

...
public class SwerveToTarget {
    ...
    public SwerveToTarget(double kP, double kV, double kA, Position[] targetPositions, double spacing, double weightSmooth, double turnSpeed, double lookaheadDistance, double pathMaximumVelocity) {
        ...
        smoothedPath = oldPathGenerator.generatePosPath(targetPositions, spacing, weightSmooth);
    ...
    public double[] calculateMotorPowers(util.Coordinate currentCoord, boolean backwards) {
        if (backwards) {
            this.currentCoord = new util.Coordinate(currentCoord.getPos(), util.Functions.normalizeAngle(currentCoord.getHeading() + 180));
        } else {
        ...
        if (indexOfClosestPoint != smoothedPath.length - 1) {
            double[] feedBack = {0.0, 0.0};/* {currentTargetWheelVelocities[0] - currentWheelVelocities[0], currentTargetWheelVelocities[1] - currentWheelVelocities[1]};*/
            ...
            double[] motorPowers = {util.Functions.constrain(feedBack[0] + feedForward[0], -1, 1), util.Functions.constrain(feedBack[1] + feedForward[1], -1, 1)};
    ...
    private double[] calculateTargetVelocities(Position[] smoothedPath, double k) {
        ...
        for (int i = smoothedPath.length - 2; i >= 0; i--) { // works backwards as we need to know last point's velocity to calculate current point's
            ...
            double distance = util.Functions.Positions.subtract(smoothedPath[i+1], smoothedPath[i]).getMagnitude();
    ...
    private double calculateCurvature(double lookaheadDistance, Position lookaheadPoint) {
        ...
        double a = -util.Functions.tand(currentCoord.getHeading());
        double b = 1;
        double c = util.Functions.tand(currentCoord.getHeading()) * currentCoord.getX() - currentCoord.getY();

        Position R = currentCoord.getPos();
        Position L = lookaheadPoint;
        // generate point B on robot line (for calculating sign)
        Position B = new Position(R.getX() + util.Functions.cosd(currentCoord.getHeading()), R.getY() + util.Functions.sind(currentCoord.getHeading()));

        Position RB = util.Functions.Positions.subtract(B, R);
        Position RL = util.Functions.Positions.subtract(L, R);

        // calculate which side of the robot line the lookahead point is on
        double side = Math.signum(util.Functions.Positions.getCross3dMagnitude(RL, RB));
